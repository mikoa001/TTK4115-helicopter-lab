 ...
 * C source code generated on : Fri Sep  5 12:52:11 2025
 *
...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    if (heli_q8_B.RateTransitionx > heli_q8_P.DeadZonex_End) {
      rtb_Frontgain = heli_q8_B.RateTransitionx - heli_q8_P.DeadZonex_End;
    } else if (heli_q8_B.RateTransitionx >= heli_q8_P.DeadZonex_Start) {
      rtb_Frontgain = 0.0;
    } else {
      rtb_Frontgain = heli_q8_B.RateTransitionx - heli_q8_P.DeadZonex_Start;
    }
     ...
     *  Gain: '<S4>/Gain: x'
     */
    heli_q8_B.Joystick_gain_x = heli_q8_P.Gainx_Gain * rtb_Frontgain *
      heli_q8_P.Joystick_gain_x;

    /* Gain: '<Root>/Gain' */
    heli_q8_B.Gain = heli_q8_P.Gain_Gain * heli_q8_B.Joystick_gain_x;
    ...
    if (heli_q8_B.RateTransitiony > heli_q8_P.DeadZoney_End) {
      rtb_Sum_p = heli_q8_B.RateTransitiony - heli_q8_P.DeadZoney_End;
    } else if (heli_q8_B.RateTransitiony >= heli_q8_P.DeadZoney_Start) {
      rtb_Sum_p = 0.0;
    } else {
      rtb_Sum_p = heli_q8_B.RateTransitiony - heli_q8_P.DeadZoney_Start;
    }
     ...
     *  Gain: '<S4>/Gain: y'
     */
    heli_q8_B.Joystick_gain_y = heli_q8_P.Gainy_Gain * rtb_Sum_p *
      heli_q8_P.Joystick_gain_y;

    /* Gain: '<Root>/Gain1' */
    heli_q8_B.Gain1 = heli_q8_P.Gain1_Gain * heli_q8_B.Joystick_gain_y;
     ...
     */
    heli_q8_B.Subtract = heli_q8_B.ElevationCounttorad -
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    rtb_TmpSignalConversionAtToFile[4] = heli_q8_B.Subtract;
      ...
      if (rtmIsMajorTimeStep(heli_q8_M) ) {
        {
          if (!(++heli_q8_DW.ToFile_IWORK.Decimation % 1) &&
              (heli_q8_DW.ToFile_IWORK.Count * (6 + 1)) + 1 < 100000000 ) {
            ...
            if (fp != (NULL)) {
              ...
              if (fwrite(u, sizeof(real_T), 6 + 1, fp) != 6 + 1) {
                rtmSetErrorStatus(heli_q8_M,
                                  "Error writing to MAT-file Test6.mat");
                return;
              }

              if (((++heli_q8_DW.ToFile_IWORK.Count) * (6 + 1))+1 >= 100000000)
              {
                (void)fprintf(stdout,
                              "*** The ToFile block will stop logging data before\n"
                              "    the simulation has ended, because it has reached\n"
                              "    the maximum number of elements (100000000)\n"
                              "    allowed in MAT-file Test6.mat.\n");
              }
            }
          }
        }
     ...
     *  Sum: '<S6>/Sum'
     */
    heli_q8_B.Kp = (heli_q8_B.Gain - heli_q8_B.PitchCounttorad) * heli_q8_P.Kpp;
  }

  /* Sum: '<S6>/Sum1' incorporates:
   *  Gain: '<S6>/Gain1'
   */
  rtb_Frontgain = heli_q8_B.Kp - heli_q8_P.Kpd * heli_q8_B.PitchTransferFcn;
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    /* Sum: '<S2>/Sum' */
    rtb_Sum_p = heli_q8_B.Gain1 - heli_q8_B.Subtract;

    /* Gain: '<S7>/K_ep' */
    heli_q8_B.K_ep = heli_q8_P.K_ep_Gain * rtb_Sum_p;
  }
   ...
   *  Sum: '<S7>/Sum'
   */
  rtb_Backgain = ((heli_q8_X.Integrator_CSTATE + heli_q8_B.K_ep) -
                  heli_q8_P.K_ed_Gain * heli_q8_B.ElevationTransferFcn) +
   ...
   *  Sum: '<S1>/Add'
   */
  rtb_Sum_p = (rtb_Backgain - rtb_Frontgain) * heli_q8_P.Frontgain_Gain;

  /* Saturate: '<S3>/Front motor: Saturation' */
  if (rtb_Sum_p > heli_q8_P.FrontmotorSaturation_UpperSat) {
    /* Saturate: '<S3>/Front motor: Saturation' */
    heli_q8_B.FrontmotorSaturation = heli_q8_P.FrontmotorSaturation_UpperSat;
  } else if (rtb_Sum_p < heli_q8_P.FrontmotorSaturation_LowerSat) {
    /* Saturate: '<S3>/Front motor: Saturation' */
    heli_q8_B.FrontmotorSaturation = heli_q8_P.FrontmotorSaturation_LowerSat;
  } else {
    /* Saturate: '<S3>/Front motor: Saturation' */
    heli_q8_B.FrontmotorSaturation = rtb_Sum_p;
  }
   ...
   *  Sum: '<S1>/Subtract'
   */
  rtb_Sum_p = (rtb_Backgain + rtb_Frontgain) * heli_q8_P.Backgain_Gain;

  /* Saturate: '<S3>/Back motor: Saturation' */
  if (rtb_Sum_p > heli_q8_P.BackmotorSaturation_UpperSat) {
    /* Saturate: '<S3>/Back motor: Saturation' */
    heli_q8_B.BackmotorSaturation = heli_q8_P.BackmotorSaturation_UpperSat;
  } else if (rtb_Sum_p < heli_q8_P.BackmotorSaturation_LowerSat) {
    /* Saturate: '<S3>/Back motor: Saturation' */
    heli_q8_B.BackmotorSaturation = heli_q8_P.BackmotorSaturation_LowerSat;
  } else {
    /* Saturate: '<S3>/Back motor: Saturation' */
    heli_q8_B.BackmotorSaturation = rtb_Sum_p;
  }
...
void heli_q8_initialize(void)
{
  ...
  {
    FILE *fp = (NULL);
    char fileName[509] = "Test6.mat";
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(heli_q8_M, "Error creating .mat file Test6.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp, 6 + 1, 0, "ans")) {
      rtmSetErrorStatus(heli_q8_M,
                        "Error writing mat file header to file Test6.mat");
      return;
    }
...
void heli_q8_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    ...
    if (fp != (NULL)) {
      char fileName[509] = "Test6.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(heli_q8_M, "Error closing MAT-file Test6.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(heli_q8_M, "Error reopening MAT-file Test6.mat");
        return;
      }

      if (rt_WriteMat4FileHeader(fp, 6 + 1, heli_q8_DW.ToFile_IWORK.Count, "ans"))
      {
        rtmSetErrorStatus(heli_q8_M,
                          "Error writing header for ans to MAT-file Test6.mat");
      }
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  /* External mode info */
  heli_q8_M->Sizes.checksums[0] = (2730806202U);
  heli_q8_M->Sizes.checksums[1] = (4262533525U);
  heli_q8_M->Sizes.checksums[2] = (1702229707U);
  heli_q8_M->Sizes.checksums[3] = (3641248846U);
  ...
  heli_q8_M->Sizes.numBlockIO = (18);  /* Number of block outputs */
  heli_q8_M->Sizes.numBlockPrms = (148);/* Sum of parameter "widths" */
  return heli_q8_M;
}
