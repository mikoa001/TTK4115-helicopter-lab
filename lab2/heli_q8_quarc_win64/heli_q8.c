...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
  ...
  for (i = 0; i < 2; i++) {
    tmp_1[i] = 0.0;
    for (i_0 = 0; i_0 < 5; i_0++) {
      tmp_1[i] += heli_q8_P.K[(i_0 << 1) + i] * tmp_0[i_0];
    }

    u[i] = (tmp[i + 2] * heli_q8_B.Gain1 + tmp[i] * heli_q8_B.Gain) - tmp_1[i];
  }
  ...
  /* '<S4>:1:7' */
  /* '<S4>:1:8' */
  heli_q8_B.Gain2 = heli_q8_P.Gain2_Gain * u[1];

  /* Gain: '<Root>/Gain3' incorporates:
   *  MATLAB Function: '<Root>/MATLAB Function'
   */
  heli_q8_B.Gain3 = heli_q8_P.Gain3_Gain * u[0];
  if (rtmIsMajorTimeStep(heli_q8_M)) {
            ...
            if (fp != (NULL)) {
              ...
              if (fwrite(u, sizeof(real_T), 6 + 1, fp) != 6 + 1) {
                rtmSetErrorStatus(heli_q8_M,
                                  "Error writing to MAT-file Test4_i.mat");
                return;
              }
              ...
              if (((++heli_q8_DW.ToFile_IWORK.Count) * (6 + 1))+1 >= 100000000)
              {
                (void)fprintf(stdout,
                              ...
                              "    allowed in MAT-file Test4_i.mat.\n");
              }
            }
          }
        }
   ...
   *  Sum: '<S1>/Add'
   */
  u0 = (heli_q8_B.Gain3 - heli_q8_B.Gain2) * heli_q8_P.Frontgain_Gain;
   ...
   *  Sum: '<S1>/Subtract'
   */
  u0 = (heli_q8_B.Gain3 + heli_q8_B.Gain2) * heli_q8_P.Backgain_Gain;
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    /* Sum: '<Root>/Sum' */
    heli_q8_B.Sum = heli_q8_B.Gain - heli_q8_B.PitchCounttorad;
  }

  /* Sum: '<Root>/Sum1' */
  heli_q8_B.Sum1 = heli_q8_B.Gain1 - heli_q8_B.ElevationTransferFcn;
}
...
void heli_q8_derivatives(void)
{
  ...
  _rtXdot->Integrator1_CSTATE = heli_q8_B.Sum1;
...
void heli_q8_initialize(void)
{
  ...
  {
    FILE *fp = (NULL);
    char fileName[509] = "Test4_i.mat";
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(heli_q8_M, "Error creating .mat file Test4_i.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp, 6 + 1, 0, "ans")) {
      rtmSetErrorStatus(heli_q8_M,
                        "Error writing mat file header to file Test4_i.mat");
      return;
    }
...
void heli_q8_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    ...
    if (fp != (NULL)) {
      char fileName[509] = "Test4_i.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(heli_q8_M, "Error closing MAT-file Test4_i.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(heli_q8_M, "Error reopening MAT-file Test4_i.mat");
        return;
      }
      ...
      {
        rtmSetErrorStatus(heli_q8_M,
                          "Error writing header for ans to MAT-file Test4_i.mat");
      }
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  /* External mode info */
  heli_q8_M->Sizes.checksums[0] = (4196458487U);
  heli_q8_M->Sizes.checksums[1] = (284970090U);
  heli_q8_M->Sizes.checksums[2] = (754327917U);
  heli_q8_M->Sizes.checksums[3] = (3681660688U);
  ...
  heli_q8_M->Sizes.numBlocks = (52);   /* Number of blocks */
  heli_q8_M->Sizes.numBlockIO = (19);  /* Number of block outputs */
  heli_q8_M->Sizes.numBlockPrms = (153);/* Sum of parameter "widths" */
  return heli_q8_M;
}
