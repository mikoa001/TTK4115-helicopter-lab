...
static int_T tSubRate(RT_MODEL * S, qsem_t * sem, int_T tid)
{
  while (1) {
     ...
     */
    if (qsem_trywait(sem) == 0 && GBLbuf.stopExecutionFlag == false) {
...
qthread_return_t QTHREAD_DECL subrate_start_routine(void * argument)
{
  ...
  /* Invoke the subrate task ensuring that task-specific termination is performed */
